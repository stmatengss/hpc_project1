	...
	for(s = 0; s < steps; s ++) {
		if (slice_x != 0) {
			MPI_Isend(A + IDX(iter_begin, 0, 0), slice_s, 
				MPI_DOUBLE, myrank - slice_sim_num2, 1, MPI_COMM_WORLD, &request );
		}
		if (slice_x != slice_sim_num - 1) {
			MPI_Isend(A + IDX(iter_end-1, 0, 0), slice_s,
				MPI_DOUBLE, myrank + slice_sum_num2, 1, MPI_COMM_WORLD, &request );
		}
		if (slice_x != 0) {
			MPI_Isend(A + IDX(iter_begin, 0, 0), slice_s, 
				MPI_DOUBLE, myrank-1, 1, MPI_COMM_WORLD, &request );
		}
		if (slice_x != slice_sim_num - 1) {
			MPI_Isend(A + IDX(iter_end-1, 0, 0), slice_s,
				MPI_DOUBLE, myrank+1, 1, MPI_COMM_WORLD, &request );
